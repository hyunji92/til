...
먼저 Android 앱은 훨씬 복잡한 구조를 가지고 있습니다. 이미지로 보실 수 있듯이 안드로이드 개발자 분이시라면 대표적으로 꼭 알아야하고 중요한 구성요소로 컴포넌트을 보실 수 있습니다. 그림 처럼 안드로이드 플랫폼안을 밑에 단까지 알고 쓴다면 더욱 많은것을 알아야하죠.

그동안  안드로이드 어플리케이션 개발을 위한 특별히 권장되는 아키텍처가 없었습니다. 그리하여 현업에 개발자들은 MVP나 MVVM,MVC 그리고 전혀 패턴이 없는 모델을 사용할 수도 있었고. 또 안드로이드 프레임 워크는 시스템 자체, 특히 구성요소의 생명주기때문에 생성된 문제에 대해 고유한 솔루션도 제공하지 않았습니다.

 하지만 위의 말씀드린 사항에도 불구하고 안드로이드의 표준 아키텍처 패턴의 부재는 오래 지속되어 왔는데요. 이제는 구글에서 공식적으로 AAC - 즉 안드로이드 아키텍처 컴포넌트를 제공하여 전세계에 안드로이드 어플리케이션의 아키텍처에 대한 공식 권장사항을 발표하고 이를 구현할 수 있도록 가이드를 제시해 주게 되었습니다.
...
코드랩을 진행하기 얼마전 AAC가 드디어 정식 지원이 되었는데요. ```Now 1.0 stable``` 업데이트 된사항에 deplicate 된 부분이 있습니다.  ( 코드로 본다 )
...
>
> 이번 코드랩을 진행할 예제는 타이머 인데요 . 화면의 상태가 변경되어도 데이터가 유지되도록 작업을 해볼껍니다.안드로이드는 화면을 세로에서 가로로 변경할때 configutation 변경으로 액티비티가 다시 실행되어 onCreate부터 다시 타게 되는데요.
>
> - 라이프 사이클 이미지와 함께 설명
...
**메모리 릭을 불러올 수 있기때문에 조심해야할 것은 뷰모델이 컨텍스트나 뷰에 강한 참조를 저장하면 안됩니다! ( context - android ViewModel) **
...
>
> 이렇게 지금까지 보신 예제를 보시면 뷰모델에서 직접 뷰를 수정하지 않고 데이터 코드만 observe관찰 하도록 해서 액티비티/프래그먼트의 구성 변경된 데이터를 수신하는것을 옵저버 패턴이라고 하죠.- 
>
...
> step4 에서는 BoundLocationManager라는 클래스에서 LifecycleRegistryOwner라는 LifecycleOwner 구현을 사용합니다. BoundLocationManager 클래스의 이름은 적은 이유는  BoundLocationManage클래스의 인스턴스가 액티비티의 라이프 사이클에 바인딩된다는 사실을 참조합니다.
>
...
옵저버는 프로 바이더의 현재 상태를 가져오기때문에, 생성자로부터 addLocationListener ()를 호출 할 필요는 없습니다. observer가 LifeCycleOwner에게 추가되면 여러분을 필요로합니다. 
